...
//Checkout.html Billing Infomation
         checkoutCtrl.prototype.BillingInfo = function(){console.log("billing countinue button on click");//console.log(self.billing);
            ...
            if(self.billing.shipping.country_id){
                var country_id = self.billing.shipping.country_id;
            } else if(self.billing.country_id){
                var country_id = self.billing.country_id;
            }  else {
                var country_id = 99;
            }
            var country_name = "India";
            if(self.billing.shipping.country_id){ 
                country_name = self.billing.shipping.country_name; //alert("hi"+country_name);
            } else if(self.billing.country_id){
                for(i=0; i<self.billing.country_list.length; i++){ //alert(self.billing.country_id);alert(self.billing.country_list[i].country_id);
                    if(self.billing.country_id == self.billing.country_list[i].country_id){ //alert("hi"); //console.log(self.billing.country_list[i]);
                        country_name = self.billing.country_list[i].name;
                        //return;
                    }
                } //console.log("mycountry"+country_name);
            }
            
//console.log("country_name666"+country_name);
